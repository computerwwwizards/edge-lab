...
- **Responsive images with picture element** - Use `<picture>` with `<source>` elements for responsive images. Define media queries in `<source>` tags to load appropriate image sizes: mobile images for `(max-width: 767px)`, tablet images for `(min-width: 768px) and (max-width: 1023px)`, desktop images for `(min-width: 1024px)`. Always include fallback `<img>` element

- **Component story required** - Every component must have at least one Storybook story that demonstrates its usage with realistic props and showcases different states or variants